...
export class ObjModelParser {
  _device: any;
  _defaultMaterial: pc.StandardMaterial;
  ...
  parse(input: string) {
      ...
      if (line[0] === 'v') {
        ...
        if (parts.length === 4) {
          ...
          for (p = 1; p < parts.length; p++) {
            ...
            if (r[1] * 2 < uvs.length) parsed[group].uvs.push(uvs[r[1] * 2], uvs[r[1] * 2 + 1]); // expand uvs from indices
            if (r[2] * 3 < normals.length)
              parsed[group].normals.push(
                normals[r[2] * 3],
                normals[r[2] * 3 + 1],
                normals[r[2] * 3 + 2],
    ...
    for (i = 0; i < groupNames.length; i++) {
      ...
      var meshOptions = {};
      if (currentGroup.normals.length > 0) meshOptions.normals = currentGroup.normals;
      if (currentGroup.uvs.length > 0) meshOptions.uvs = currentGroup.uvs;
