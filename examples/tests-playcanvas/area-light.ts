...
export async function areaLight(canvas: any) {
  ...
  const assets = labelAssets(
    await loadAssets(app, [
      { url: getUrl('static/assets/textures/seaside-rocks01-color.jpg'), type: 'texture' },
      { url: getUrl('static/assets/textures/seaside-rocks01-normal.jpg'), type: 'texture' },
      { url: getUrl('static/assets/textures/seaside-rocks01-gloss.jpg'), type: 'texture' },
      { url: getUrl('static/assets/models/statue.glb'), type: 'container' },
      { url: getUrl('static/assets/binary/area-light-luts.bin'), type: 'binary' },
    ]),
    ['color', 'normal', 'gloss', 'statue', 'luts'],
  );
  ...
  function createAreaLight(
    ...
    brightShape.addComponent('render', {
      type:
        shape === pc.LIGHTSHAPE_SPHERE ? 'sphere' : shape === pc.LIGHTSHAPE_DISK ? 'cone' : 'plane',
