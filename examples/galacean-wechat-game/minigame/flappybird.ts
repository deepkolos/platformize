...
export async function FlappyBird(canvas: any) {
  enum EnumBirdState {
    Alive = 0,
    Dead = 1,
  }
  ...
  camera.orthographicSize = engine.canvas.height / 100 / 2;
  ...
  engine.resourceManager
    ...
    .then((texture2DArr: Object[]) => {
      // Record the resources.
      gameResArray = texture2DArr as Texture2D[];
      ...
      nodeBg.addComponent(SpriteRenderer).sprite = new Sprite(engine, texture2DArr[0] as Texture2D);
      ...
      groundMaterial.baseTexture = texture2DArr[2] as Texture2D;
      ...
      nodeBird.addComponent(SpriteRenderer).sprite = new Sprite(engine, texture2DArr[3] as Texture2D);
      ...
      nodeRestart.addComponent(SpriteRenderer).sprite = new Sprite(engine, texture2DArr[4] as Texture2D);
      // Score.
  ...
  class ScriptPipe extends Script {
      ...
      engine.on(GameEvent.stateChange, (gameState: EnumGameState) => {
        switch (gameState) {
          ...
          case EnumGameState.Start:
            this.enabled = true;
            this._curStartTimeStamp = engine.time.elapsedTime;
            break;
    ...
    onUpdate(deltaTime: number) {
      ...
      if (engine.time.elapsedTime - this._curStartTimeStamp >= debutTime) {
        let bAddScore = false;
    ...
    private _createPipe(posX: number, posY: number, posZ: number) {
      ...
      this.entity.addChild(pipe as Entity);
      this._nowPipeArr.push(pipe as Entity);
    }
  ...
  class GameCtrl extends Script {
    ...
    onAwake() {
      ...
      boxColliderShape.size = new Vector3(10, 10, 0.001);
  ...
  class ScriptGUI extends Script {
    onAwake() {
      ...
      const boxColliderShape = new BoxColliderShape();
      boxColliderShape.size = new Vector3(2.14, 0.75, 0.001);
      boxCollider.addShape(boxColliderShape);
  ...
  class ScriptBird extends Script {
      ...
      switch (this._birdState) {
        case EnumBirdState.Alive:
          ...
          if (this._gameState == EnumGameState.Start) {
            ...
            const subTime = (engine.time.elapsedTime - this._flyStartTime);
            // How long has it been in free fall.
            const addToMaxUseTime = (_maxDropV - _startFlyV) / _gravity;
            if (subTime <= addToMaxUseTime) {
              // Free fall.
              endY =
                ((_startFlyV + (_startFlyV + subTime * _gravity)) * subTime) / 2 + this._startY;
            } else {
    ...
    private _initListener() {
      ...
      engine.on(GameEvent.fly, () => {
        ...
        this._flyStartTime = engine.time.elapsedTime;
        // Flying performance.
